...
class ModelBEGAN:
	...
	def __defineModel(self):
		...
		self.gen_image_uint8 = gen_image_uint8 = tf.image.convert_image_dtype(gen_image, tf.uint8, name='gen_image_uint8')
		...
		## Loss
		k = tf.get_variable('k', shape=[], dtype=tf.float32, initializer=tf.constant_initializer(0.0), trainable=False)

		loss_real_image = reconLoss(real_image, restored_real_image, 'real_image')
		loss_gen_image = reconLoss(gen_image, restored_gen_image, 'gen_image')
		...
		loss_discriminator = loss_real_image - k * loss_gen_image
	...
	def __defineGenerator(self, z, reuse_scope=False):
		with tf.variable_scope('generator') as scope:
			...
			def doConv(input, filters):
				output = tf.layers.conv2d(input, filters, [3, 3], padding='SAME')
			...
			with tf.variable_scope('output'):
				output = tf.layers.conv2d(conv4_2, 3, [3, 3], padding='SAME')
	...
	def __defineDiscriminator(self, image, reuse_scope=False):
		...
		with tf.variable_scope('discriminator') as scope:
			...
			with tf.variable_scope('decoder'):
				...
				with tf.variable_scope('output'):
					output = tf.layers.conv2d(dec_conv4_2, 3, [3, 3], padding='SAME')
					output = tf.nn.sigmoid(output, name='output')
	...
	def __testFast(self, sess):
		...
		self.data.saveImage('training', '0', gen_image[index])
	...
	def train(self, savepoint=None):
		...
		with tf.Session(config=config) as sess:
			...
			for epoch in range(0, 1000, 1):
				...
				percent = 1
				for step in range(0, len(train_pair_list) - self.batch_size, self.batch_size):
					feed_dict = self.__prepareTrainFeedDictList(train_pair_list, step)
					_, _, convergence_measure = sess.run([self.train_step, self.update_k, self.convergence_measure], feed_dict=feed_dict)
